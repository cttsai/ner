	...
	double Cp, double Cn, int solver_type)
{
	...
	for(i=0; i<l; i++)
	{
		...
		while(xi->index != -1)
		{
			double val_sq = (xi->value)*(xi->value);
			tmp_value += val_sq;
			feature_node *xj = xi + 1;
			double *w_x = &w[((xi->index)*(2*n-xi->index+1))/2];
			w_x[xi->index] += y_alpha*val_sq;
			while(xj->index != -1)	
			{
				w_x[xj->index] += sqrt2_y_alpha*(xi->value)*(xj->value);
				xj++;
			}
			w[xi->index] += sqrt2_y_alpha*coef0*(xi->value);
			xi++;
		}
	...
	int solver_type)
{
	...
	for(i=0; i<l; i++)
	{
		...
		while(xi->index != -1)
		{
			...
			feature_node *xj = xi + 1;
			double *w_x = &w[((xi->index)*(2*n-xi->index+1))/2];
			w_x[xi->index] += beta[i]*val_sq;
			while(xj->index != -1)	
			{
				w_x[xj->index] += sqrt2_beta*(xi->value)*(xj->value);
				xj++;
			}
			w[xi->index] += sqrt2_beta*coef0*(xi->value);
			xi++;
		}
		tmp_value = gamma*tmp_value+coef0;
		QD[i] += tmp_value*tmp_value;
...
#ifdef POLY2
		xTx = (param->gamma*xTx + param->coef0)*(param->gamma*xTx + param->coef0);
#endif
		if(xTx > max_xTx)
			max_xTx = xTx;
	}
...
#ifdef POLY2
		subprob[i].coef0 = prob->coef0;
		subprob[i].gamma = prob->gamma;
#endif
...
const char *check_parameter(const problem *prob, const parameter *param)
{
	...
	if(param->solver_type != L2R_L2LOSS_SVC_DUAL
		...
		return "Currently only support L2-regularized L1- and L2-loss SVC/SVR";
#endif
